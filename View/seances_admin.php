...
<body>
    ...
    <h1 id = "title"> <a href = "./cinema.php"> My cinema </a> </h1>
        <div>
            <ul>
                <li> 
                    <a href = "./abonnement_admin.php"> Abonnement </a> 
                </li>
                <li> Séance </li>
                <li> Historique </li>
                <li id = "search">
    ...
    <section>
        ...
        ?>
        <form method = "post" action = "">
            <input type = "text" name = "add_seance">
            <button type = "submit" value = "Add_Seance"> Search </button>
        </form>
        <?php
        $search_add = $_REQUEST["add_seance"];
        $films = $connect -> query("SELECT * FROM movie WHERE title LIKE '%$search_add%'");
        $films -> fetch_all(MYSQLI_ASSOC);
        if ($search_add != "")
        {
            foreach ($films as $film)
            {
                ?>
                <div class = "flex">
                    <p class = "film"> <strong><i><?php echo "$film[title]" ;?></i></strong></p>
                    <form method = "post" action = "">
                        <button type = "submit" value = "Ajouter" name = "Ajouter"> Ajouter </button>
                        <input type = 'hidden' value = '<?php echo $film['id']; ?>' name = 'hidden_movie'>
                    </form>
                </div>
                <?php
            }
        ...
        if (isset($_POST['Ajouter'])) 
        {
            ...
            if (!empty($id_movie)) 
            {
                $add = $connect -> query("INSERT INTO movie_schedule(id_movie, id_room, date_begin) VALUES('$id_movie', '3', NOW())");
                echo "Séance de film ajouté avec succès!";
            } 
            else 
            {
                echo "Erreur lors de l'ajout de la séance du film.";
            }
        }
        ?>
    </section>
